...
class APIManager:
    ...
    def cohere_api(smartcontract_content, test_result):
        body = APIManager._generate_api_content(smartcontract_content, test_result)
        response = cohere.Client(APIManager.cohere_api_key).chat(body, model="command", temperature=0.9)
    ...
    def llama_api(smartcontract_content, test_result):
        content = APIManager._generate_api_content(smartcontract_content, test_result)
        api_request_json = {
            "messages": [{"role": "user", "content": content}],
            "stream": False,
        }
        response = LlamaAPI(APIManager.llama_api_key).run(api_request_json)
