...
class APIManagerZad1:
    ...
    def cohere_api(smartcontract_content, test_result):
        body = APIManagerStaticToDynamic._generate_api_content(smartcontract_content, test_result)
        response = cohere.Client(APIManagerStaticToDynamic.cohere_api_key).chat(body, model="command", temperature=0.9)
    ...
    def llama_api(smartcontract_content, test_result):
        content = APIManagerStaticToDynamic._generate_api_content(smartcontract_content, test_result)
        api_request_json = {
            "messages": [{"role": "user", "content": content}],
            "stream": False,
        }
        response = LlamaAPI(APIManagerStaticToDynamic.llama_api_key).run(api_request_json)
...
class APIManagerStaticToDynamic:
    static_testing_method = "Solhint"
    dynamic_testing_method = "Manticore"
    cohere_api_key = 'nasXJmj9GyZF0hH7LsS3CwXoVqrEqc4QlolOAM7B'
    llama_api_key = 'LL-4Upqf2Vo34LciTSHPNG44yNuByGyvoxXATFaZ5CaktJWp42OgkHcO4VQROLP2LLT'

    @staticmethod
    def cohere_api(smartcontract_content, test_result):
        print("COHERE api start...")
        body = APIManagerStaticToDynamic._generate_api_content(smartcontract_content, test_result)
        response = cohere.Client(APIManagerStaticToDynamic.cohere_api_key).chat(body, model="command", temperature=0.9)
        answer = response.text
        # print(answer)
        print("COHERE api end.")
        return answer

    @staticmethod
    def llama_api(smartcontract_content, test_result):
        print("LAMA api start...")
        content = APIManagerStaticToDynamic._generate_api_content(smartcontract_content, test_result)
        api_request_json = {
            "messages": [{"role": "user", "content": content}],
            "stream": False,
    ...
    @staticmethod
    def _generate_api_content(smartcontract_content, test_result):
        return f"Mając ten smartkontrakt: `{smartcontract_content}`, oraz wynik testów tego smartkontraktu przy użyciu narzędzia {APIManagerStaticToDynamic.static_testing_method}: `{test_result}`, zaproponuj testy dynamiczne przy użyciu narzędzia {APIManagerStaticToDynamic.dynamic_testing_method}"